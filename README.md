...
- springdoc-openapi (Swagger UI para probar endpoints — ver sección siguiente)
- MySQL
- Maven
...
## Probar endpoints (Swagger UI)

Con la app corriendo, la API expone Swagger UI para probar los endpoints desde el navegador:

| URL | Qué es |
|-----|--------|
| `http://localhost:8080/swagger-ui.html` | Swagger UI: doc navegable + probar endpoints |
| `http://localhost:8080/v3/api-docs` | La especificación OpenAPI en JSON |

Pasos:

1. Abrir `http://localhost:8080/swagger-ui.html`.
2. Ejecutar `POST /sportcenter/auth/login` (**Try it out** → completar email/password → **Execute**) y copiar el `token` de la respuesta.
3. Click en **Authorize** (candado, arriba a la derecha), pegar el token (sin el prefijo `Bearer`, lo agrega solo) y confirmar.
4. Desde ahí, todos los **Try it out** mandan el `Authorization: Bearer <token>` automáticamente.
